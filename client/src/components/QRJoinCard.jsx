    ...
    >
      <div>
        ...
        <p className="mt-3 max-w-sm text-sm text-slate-300">
          Escaneie o QR Code ou digite o endereco exibido abaixo no navegador do celular.
        </p>
      </div>

      <div className="my-6 flex items-center justify-center rounded-[2rem] bg-white p-5 shadow-neon">
        {joinUrl ? <QRCode value={joinUrl} size={210} /> : <span className="text-slate-900">QR indisponivel</span>}
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-3xl border border-white/10 bg-slate-950/35 p-4">
          <p className="text-xs uppercase tracking-[0.3em] text-slate-400">Endereco para o celular</p>
          <p className="mt-2 break-all text-sm font-semibold leading-6 text-slate-100">
            {visibleJoinUrl || "URL indisponivel"}
          </p>
        </div>
